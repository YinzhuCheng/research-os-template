...
Use this skill when turning accepted research outputs into a manuscript and `dissemination.paper_enabled` is true, the researcher explicitly asks for paper writing, or a `final_product_plan` selects the paper track.

## Workflow

1. Read the `final_product_plan` when present, dissemination target, venue or journal rules if provided, claim-evidence matrix, result analysis, figure plan, and appendix plan.
...
7. If no paper target is enabled or selected, stop and create a report, software, or decision memo work order instead.

## Quick Checks

- Read `references/latex_authoring.md` before writing manuscript text.
- `PUBLIC/paper/` is not a default tracked scaffold. Create it only after `dissemination.paper_enabled: true`, a selected paper final-product track, or an explicit paper work order.
