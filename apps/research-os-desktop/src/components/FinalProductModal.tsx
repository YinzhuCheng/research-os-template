...
const tracks = [
  ...
  { id: "report", label: "研究报告", description: "更详细记录过程、初始数据、负结果、复现细节。", Icon: Presentation },
  { id: "software", label: "软件", description: "正式、稳定、规范、美观、用户友好，并保留中间产物。", Icon: PackageCheck }
];

export function FinalProductModal({ open, onClose, onSubmit }: Props) {
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="final-product-title">
        <button className="icon-button modal-close" type="button" aria-label="关闭最终产物选择" onClick={onClose}>
          <X size={18} />
        </button>
        <h2 id="final-product-title">进入最终产物阶段</h2>
        <p className="muted">可多选产物类型；关闭弹窗会继续停留在第二阶段。</p>
        ...
        <div className="modal-actions">
          ...
          <button className="primary-action" type="button" onClick={() => onSubmit(selected, freeForm)} disabled={selected.length === 0}>
            确认进入
          </button>
        </div>
      </section>
